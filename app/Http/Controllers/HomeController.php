...
class HomeController extends Controller
{
    public function index()
    {
        if (auth()->user()->role == 'admin') {
            ...
            $data['latest_laporan_akhir'] = LaporanAkhir::with(['user','laporanKemajuan.proposal'])->latest()->take(5)->get();
        }
        else
        {
            ...
            $data['latest_laporan_akhir'] = LaporanAkhir::with(['user','laporanKemajuan.proposal'])->where('user_id', auth()->user()->id)->latest()->take(5)->get();
        }
